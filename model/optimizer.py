...
class Optimizer:
    ...
    def run(self, progress_bar: bool = False, prefilter: bool = False):
        if prefilter:
            self.prefilter_nodes(load_precomputed=True)
        ...
        for i in iterator:
            ...
            costs = Evaluator(self.matrix, allocations, nodes).get_costs()
    ...
    def prefilter_nodes(self, load_precomputed: bool = True, progress_bar: bool = True):
        if load_precomputed:
            ...
            reps = 10
            base_optimization_iter_mult = 0.2
            base_iter = int(base_optimization_iter_mult * self.n_iter)
            best_team_constellations = []
            for rep in range(reps):
                rep_costs = math.inf
                rep_best_teams = None
                for i in tqdm(range(base_iter)) if progress_bar else range(base_iter):
                    allocator = Allocator(self.matrix, self.n_rounds, self.n_locations)
                    allocations, nodes = allocator.get_feasible_solution()
                    costs = Evaluator(self.matrix, allocations, nodes).get_costs()
                    if costs < rep_costs:
                ...
                best_team_constellations.append(rep_best_teams)
